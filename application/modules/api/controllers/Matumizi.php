...
class Matumizi extends REST_Controller {

    function __construct(){
        parent::__construct();
        $this->load->helper('date');
        $this->load->module('Admin');
        $this->load->model('Mdl_matumizi');
    }
...
function users_get(){
    $table= 'user';
        $users = $this->Mdl_matumizi->get_users('id',$table)->result();
        if($users){
...
function cpwrd_get() {
    ...
    $res = $this->Mdl_matumizi->_update2($table, 'username', $username, $data);
    if($data){
...
function login_get() {
    ...
    $username = $this->get('username');
    $pwrd = $this->get('pwrd');
    $chckres = $this->Mdl_matumizi->get_where_custom_tb($table, 'username', $username);
...
function userreg_get() {
    ...
    if ($chckres->num_rows() > 0) {
        ...
        $res = $this->Mdl_matumizi->_insert_tb($table, $data);
        $feedback = "Done";
    }
...
function expenseup_get() {
    ...
    $data = array('userid'=>$userid, 'amount'=>$amount, 'reason'=>$reason, 'time'=>$time, 'day'=>$day, 'month'=>$month, 'year'=>$year,'date'=>$date, 'udate'=>$udate ); 
    $chckres = $this->Mdl_matumizi->get_where_custom_tb('user', 'id', $userid);
    if ($chckres->num_rows() > 0) {
        # code...
        //check if record exists
        $chk = $this->Mdl_matumizi->get_where_custom3($table, 'userid', $userid, 'date', $date, 'time', $time);
        if($chk->num_rows() == 0) {
            $res = $this->Mdl_matumizi->_insert_tb($table, $data);
        } else {
            $res = $this->Mdl_matumizi->_update_tb($table, $chk->row()->id, $data);
        } //end dada exist check
...
function expensedown_get() {
    ...
    if ($chckres->num_rows() > 0) {
        # code...
        $res = $this->Mdl_matumizi->get_where_custom_tb($table, 'userid', $userid);
        if ($res->num_rows() > 0) {
            $feedback = $res->result();;
        } else {$feedback = "Empty";}
...
// START BUDGET.........................
function budgetup_get() {
    $table = 'budget';
    $userid = $this->get('userid');
    $bgt_amount= $this->get('bgt_amount');
    $exp_amount= $this->get('exp_amount');
    /*$reason= $this->get('reason');
    $time = $this->get('time');
    $day= $this->get('day');*/
    $month= $this->get('month');
    ...
    $data = array('userid'=>$userid, 'bgt_amount'=>$bgt_amount, 'exp_amount'=>$exp_amount, 'month'=>$month, 'year'=>$year,'date'=>$date, 'udate'=>$udate ); 
    $chckres = $this->Mdl_matumizi->get_where_custom_tb('user', 'id', $userid);
    if ($chckres->num_rows() > 0) {
        # code...
        //check if record exists
        $chk = $this->Mdl_matumizi->get_where_custom3($table, 'userid', $userid, 'month', $month, 'year', $year);
        if($chk->num_rows() == 0) {
            $res = $this->Mdl_matumizi->_insert_tb($table, $data);
        } else {
            $res = $this->Mdl_matumizi->_update_tb($table, $chk->row()->id, $data);
...
// START INCOME.........................
function incomeup_get() {
    $table = 'income';
    $userid = $this->get('userid');
    $in_amount= $this->get('in_amount');
    ...
    $data = array('userid'=>$userid, 'in_amount'=>$in_amount, 'source'=>$source, 'time'=>$time, 'day'=>$day, 'month'=>$month, 'year'=>$year,'date'=>$date, 'udate'=>$udate ); 
    $chckres = $this->Mdl_matumizi->get_where_custom_tb('user', 'id', $userid);
    if ($chckres->num_rows() > 0) {
        # code...
        //check if record exists
...
function codedown_get() {
    ...
    $chckres = $this->Mdl_matumizi->get_where_custom2($table, 'code', $code, 'status', 'Available');
    if ($chckres->num_rows() > 0) {
        # code...
        $data = array('status'=> "Used");
        $update = $this->Mdl_matumizi->_update2($table, 'id', $chckres->row()->id, $data );
        $res = $chckres;
        $feedback = $res->result();;
    } else {
         $feedback = "Failed";
...
function pmans_get(){
    ...
    $res = $this->Mdl_matumizi->get_tb($table, 'accessCode')->result();
    if($res){
...
function sitelogin_get() {
    ...
    //create access code
    $access_code_res = $this->Mdl_matumizi->get_tb('accesscode', 'idnumber');
    $access_code = $access_code_res->row()->accessCode_number + 1;
    $cdata['accessCode_number'] = $access_code;
    $this->Mdl_matumizi->_update2('accesscode', ' idnumber',  $access_code_res->row()->idnumber, $cdata);
...
//"allopen="+allopen+"&accessCode="+accessCode+"&site_id="+site_id+"&user_id="+user_id+"&dg_fuel="+dg_fuel+"&hybridtype="+hybridtype+"&hybridstatus="+hybridstatus+"&luku_Balance="+luku_Balance+"&dgRunHour="+nHour+"&mainspowerfrailer="+mainspowerfrailer+"&batterydischarge="+batterydischarge+"&hightemperature="+hightemperature+"&commondgalarm="+commondgalarm+"&generatorrunng="+generatorrunng+"&generatorlowfuel="+generatorlowfuel+"&dgbatterystatus="+dgbatterystatus+"&rectifierType="+rectifierType+"&moduleNo="+moduleNo+"&noWorking="+noWorking+"&notWorking="+notWorking+"&loadCurrent="+loadCurrent+"&powerConfiguration="+powerConfiguration+"&shelterStatus="+shelterStatus+"&autonomyBTS="+autonomyBTS+"&autonomyTX="+autonomyTX+"&remarks="+remarks+"&log_out_time="+log_out_time+"&status="+status+"&closingAgent="+closingAgent
    ...
    $site_id  = $this->get('site_id');
    $user_id = $this->get('user_id');
    $closingAgent  = $this->Mdl_matumizi->get_where_custom_tb('user_info', 'user_ID', $user_id)->row()->fname+" "+$this->Mdl_matumizi->get_where_custom_tb('user_info', 'user_ID', $user_id)->row()->sname+" "+$this->Mdl_matumizi->get_where_custom_tb('user_info', 'user_ID', $user_id)->row()->lname; //read name from users table
    ...
    if ($allopen == "yes") {
        ...
        $res = $this->Mdl_matumizi->_update2( $table, 'site_id', $site_id, $data);
        if($data){
            $this->response($data, 200);
        }else{
            $this->response(NULL, 404);
        ...
        # code...
         $data =array('dg_fuel' => $this->get('dg_fuel') ,'hybridtype' => $this->get('hybridtype') ,'hybridstatus' => $this->get('hybridstatus') ,'luku_Balance' => $this->get('luku_Balance') ,'dgRunHour' => $this->get('dgRunHour') ,'mainspowerfrailer' => $this->get('mainspowerfrailer') ,'batterydischarge' => $this->get('batterydischarge') ,'hightemperature' => $this->get('hightemperature') ,'commondgalarm' => $this->get('commondgalarm') ,'generatorrunng' => $this->get('generatorrunng') ,'generatorlowfuel' => $this->get('generatorlowfuel') ,'dgbatterystatus' => $this->get('dgbatterystatus'),  'rectifierType' => $this->get('rectifierType') ,'moduleNo' => $this->get('moduleNo') ,'noWorking' => $this->get('noWorking') ,'notWorking' => $this->get('notWorking') ,'loadCurrent' => $this->get('loadCurrent') ,'powerConfiguration' => $this->get('powerConfiguration') ,'shelterStatus' => $this->get('shelterStatus') ,'autonomyBTS' => $this->get('autonomyBTS') ,'autonomyTX' => $this->get('autonomyTX') ,'remarks' => $this->get('remarks') ,'log_out_time' => $this->get('log_out_time') ,'status' => $this->get('status') ,'closingAgent' => $closingAgent ); 
        $res = $this->Mdl_matumizi->_update2( $table, 'accessCode', $accessCode, $data);
        if($data){
            $this->response($data, 200);
        }else{
            $this->response(NULL, 404);
...
function fuel_get(){
    $table = 'fuelrequest';
    $res = $this->Mdl_matumizi->get_tb($table, 'reqID')->result();
    if($res){
        $this->response($res, 200);
    }else{
        $this->response(NULL, 404);
...
function site_get(){
    $table = 'site';
    $res = $this->Mdl_matumizi->get_tb($table, 'site_id')->result();
    if($res){
        $this->response($res, 200);
    }else{
        $this->response(NULL, 404);
