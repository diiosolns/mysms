...
class Mauzo extends REST_Controller {
...
public function index_get(){
  ...
  $this->response($this->db->get('mau_user')->result());
}
...
function users_get(){
    $table= 'mau_user';
        $users = $this->Mdl_mauzo->get_users('id',$table)->result();
        if($users){
...
function cpwrd_get() {
    ...
    $res = $this->Mdl_mauzo->_update2($table, 'username', $username, $data);
    if($data){
...
function login_get() {
    ...
    $username = $this->get('username');
    $pwrd = $this->get('pwrd');
    $chckres = $this->Mdl_mauzo->get_where_custom_tb($table, 'username', $username);
...
function userreg_get() {
    ...
    $pwrd = $this->get('pwrd');
    $date = mdate('%Y-%m-%d');
    //$data = array('phone'=>$phone, 'email'=>$email, 'username'=>$username, 'password'=>$pwrd, 'udate'=>$date); 
    ...
    if ($chckres->num_rows() > 0) {
        ...
        $res = $this->Mdl_mauzo->_insert_tb($table, $data);
        $feedback = "Done";
    }
...
function expenseup_get() {
    ...
    $data = array('userid'=>$userid, 'amount'=>$amount, 'qnty'=>$qnty, 'reason'=>$reason, 'time'=>$time, 'day'=>$day, 'month'=>$month, 'year'=>$year,'date'=>$date, 'udate'=>$udate ); 
    $chckres = $this->Mdl_mauzo->get_where_custom_tb('mau_user', 'id', $userid);
    if ($chckres->num_rows() > 0) {
        # code...
        $ext_chckres = $this->Mdl_mauzo->get_where_custom3($table, 'userid', $userid, 'date', $date, 'time', $time);
        if ($ext_chckres->num_rows() > 0) {
            $res = $this->Mdl_mauzo->_update_tb($table, $ext_chckres->row()->id, $data);
            $feedback = "Done";
        } else {
            $res = $this->Mdl_mauzo->_insert_tb($table, $data);
            $feedback = "Done";
...
function expensedown_get() {
    ...
    if ($chckres->num_rows() > 0) {
        # code...
        $res = $this->Mdl_mauzo->get_where_custom_tb($table, 'userid', $userid);
        if ($res->num_rows() > 0) {
            $feedback = $res->result();;
        } else {$feedback = "Empty";}
...
function itemup_get() {
    ...
    $data = array('userid'=>$userid, 'name'=>$name, 'qnty'=>$qnty, 'cost'=>$cost, 'price'=>$price,'date'=>$date, 'udate'=>$udate ); 
    $chckres = $this->Mdl_mauzo->get_where_custom_tb('mau_user', 'id', $userid);
    if ($chckres->num_rows() > 0) {
        # code...
         $ext_chckres = $this->Mdl_mauzo->get_where_custom2($table, 'userid', $userid, 'name', $name);
        if ($ext_chckres->num_rows() > 0) {
            $res = $this->Mdl_mauzo->_update_tb($table, $ext_chckres->row()->id, $data);
            $feedback = "Done";
        } else {
...
function incomeup_get() {
   ...
   /* $date = mdate('%Y-%m-%d');
    ...
    $data = array('userid'=>$userid, 'in_amount'=>$in_amount, 'profit'=>$profit, 'qnty'=>$qnty, 'source'=>$item, 'time'=>$time, 'day'=>$day, 'month'=>$month, 'year'=>$year,'date'=>$date, 'udate'=>$udate ); 
    $chckres = $this->Mdl_mauzo->get_where_custom_tb('mau_user', 'id', $userid);
    if ($chckres->num_rows() > 0) {
        # code...
         $ext_chckres = $this->Mdl_mauzo->get_where_custom3($table, 'userid', $userid, 'date', $date, 'time', $time);
...
function codedown_get() {
    ...
    $chckres = $this->Mdl_mauzo->get_where_custom2($table, 'code', $code, 'status', 'Available');
    if ($chckres->num_rows() > 0) {
        # code...
        $data = array('status'=> "Used");
        $update = $this->Mdl_mauzo->_update2($table, 'id', $chckres->row()->id, $data );
        $res = $chckres;
        $feedback = $res->result();;
    } else {
         $feedback = "Failed";
...
function pmans_get(){
    ...
    $res = $this->Mdl_mauzo->get_tb($table, 'accessCode')->result();
    if($res){
...
function sitelogin_get() {
    ...
    //create access code
    $access_code_res = $this->Mdl_mauzo->get_tb('accesscode', 'idnumber');
    $access_code = $access_code_res->row()->accessCode_number + 1;
    $cdata['accessCode_number'] = $access_code;
    $this->Mdl_mauzo->_update2('accesscode', ' idnumber',  $access_code_res->row()->idnumber, $cdata);
...
//"allopen="+allopen+"&accessCode="+accessCode+"&site_id="+site_id+"&user_id="+user_id+"&dg_fuel="+dg_fuel+"&hybridtype="+hybridtype+"&hybridstatus="+hybridstatus+"&luku_Balance="+luku_Balance+"&dgRunHour="+nHour+"&mainspowerfrailer="+mainspowerfrailer+"&batterydischarge="+batterydischarge+"&hightemperature="+hightemperature+"&commondgalarm="+commondgalarm+"&generatorrunng="+generatorrunng+"&generatorlowfuel="+generatorlowfuel+"&dgbatterystatus="+dgbatterystatus+"&rectifierType="+rectifierType+"&moduleNo="+moduleNo+"&noWorking="+noWorking+"&notWorking="+notWorking+"&loadCurrent="+loadCurrent+"&powerConfiguration="+powerConfiguration+"&shelterStatus="+shelterStatus+"&autonomyBTS="+autonomyBTS+"&autonomyTX="+autonomyTX+"&remarks="+remarks+"&log_out_time="+log_out_time+"&status="+status+"&closingAgent="+closingAgent
    ...
    $site_id  = $this->get('site_id');
    $user_id = $this->get('user_id');
    $closingAgent  = $this->Mdl_mauzo->get_where_custom_tb('user_info', 'user_ID', $user_id)->row()->fname+" "+$this->Mdl_mauzo->get_where_custom_tb('user_info', 'user_ID', $user_id)->row()->sname+" "+$this->Mdl_mauzo->get_where_custom_tb('user_info', 'user_ID', $user_id)->row()->lname; //read name from users table
    ...
    if ($allopen == "yes") {
        ...
        $res = $this->Mdl_mauzo->_update2( $table, 'site_id', $site_id, $data);
        if($data){
            $this->response($data, 200);
        }else{
            $this->response(NULL, 404);
        ...
        # code...
         $data =array('dg_fuel' => $this->get('dg_fuel') ,'hybridtype' => $this->get('hybridtype') ,'hybridstatus' => $this->get('hybridstatus') ,'luku_Balance' => $this->get('luku_Balance') ,'dgRunHour' => $this->get('dgRunHour') ,'mainspowerfrailer' => $this->get('mainspowerfrailer') ,'batterydischarge' => $this->get('batterydischarge') ,'hightemperature' => $this->get('hightemperature') ,'commondgalarm' => $this->get('commondgalarm') ,'generatorrunng' => $this->get('generatorrunng') ,'generatorlowfuel' => $this->get('generatorlowfuel') ,'dgbatterystatus' => $this->get('dgbatterystatus'),  'rectifierType' => $this->get('rectifierType') ,'moduleNo' => $this->get('moduleNo') ,'noWorking' => $this->get('noWorking') ,'notWorking' => $this->get('notWorking') ,'loadCurrent' => $this->get('loadCurrent') ,'powerConfiguration' => $this->get('powerConfiguration') ,'shelterStatus' => $this->get('shelterStatus') ,'autonomyBTS' => $this->get('autonomyBTS') ,'autonomyTX' => $this->get('autonomyTX') ,'remarks' => $this->get('remarks') ,'log_out_time' => $this->get('log_out_time') ,'status' => $this->get('status') ,'closingAgent' => $closingAgent ); 
        $res = $this->Mdl_mauzo->_update2( $table, 'accessCode', $accessCode, $data);
        if($data){
            $this->response($data, 200);
        }else{
            $this->response(NULL, 404);
...
function fuel_get(){
    $table = 'fuelrequest';
    $res = $this->Mdl_mauzo->get_tb($table, 'reqID')->result();
    if($res){
        $this->response($res, 200);
    }else{
        $this->response(NULL, 404);
...
function site_get(){
    $table = 'site';
    $res = $this->Mdl_mauzo->get_tb($table, 'site_id')->result();
    if($res){
        $this->response($res, 200);
    }else{
        $this->response(NULL, 404);
